...
function load_game_data() {
  ...
  .then(files => {
    for (var file of files) {
      for (var line of file.split('\n')) {
        ...
        gameId = parseInt(gameId)
        if (globalGameData[gameId] == null) continue // ??? invalid data
        for (var similarGame of similarGames.split(',')) {
          similarGame = parseInt(similarGame)
          if (globalGameData[similarGame] == null) continue // ??? invalid data

          globalGameData[gameId].similar.add(similarGame)
          globalGameData[similarGame].reverse.add(gameId)
        }
      }
    }
  })
  ...
  .then(r => {
    for (var line of r.split('\n')) {
      ...
      gameId = parseInt(gameId)
      if (globalGameData[gameId] == null) continue // ??? invalid data
      globalGameData[gameId].tags = new Set(tags.split(',').map(t => parseInt(t)))
    }
  })
  ...
  .then(r => {
    for (var line of r.split('\n')) {
      ...
      gameId = parseInt(gameId)
      if (globalGameData[gameId] == null) continue
      globalGameData[gameId].total = total
      globalGameData[gameId].perc = positive / total
    }
  })
}
